...
class _TianKeyHomeState extends State<TianKeyHome> {
  ...
  bool get borrowValid {
    if (borrowCode == null || borrowStart == null || borrowEnd == null) {
      return false;
    }
  ...
  bool get adminEnabled =>
      connected && mode == AccessMode.admin && adminSession;

  bool get vehicleEnabled =>
      connected &&
      authorized &&
        ...
        Column(
          ...
          children: <Widget>[
            _actionButton(
              ...
              const Color(0xFF1595FF),
              () => Navigator.pop(context, AccessMode.admin),
            ),
            ...
            _actionButton(
              ...
              () => Navigator.pop(context, AccessMode.borrower),
            ),
          ],
        ),
      ),
  ...
  void vehicleCommand(String command) {
    ...
    setState(() {
      status = '$command 已发送：$lastCommand';
    });
    _log('APP发起 $protocol');
  ...
  Future<void> generateBorrowCode() async {
    ...
    final hours = (int.tryParse(hoursController.text.trim()) ?? 2)
        .clamp(1, 24)
        .toInt();
  ...
  InputDecoration _field(String label) {
    return InputDecoration(
      ...
      enabledBorder: const OutlineInputBorder(
        borderSide: BorderSide(color: Color(0xFF1595FF)),
      ),
      focusedBorder: const OutlineInputBorder(
        borderSide: BorderSide(color: Color(0xFFFF8A1C), width: 2),
      ),
    );
  }

  Widget _dialog(String title, Widget child) {
    return Dialog(
      backgroundColor: const Color(0xFF06101D),
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(18),
        side: const BorderSide(color: Color(0xFF1595FF)),
      ),
      child: Padding(
  ...
  Widget _panel(
    Widget child, {
    Color border = const Color(0xFF1595FF),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: <BoxShadow>[
          BoxShadow(color: border.withOpacity(0.28), blurRadius: 14),
        ],
      ),
      child: child,
    );
  ...
  Widget _line(String label, String value, {bool green = false}) {
    return Padding(
      ...
      child: Row(
        children: <Widget>[
          ...
          Text(
            value,
            style: TextStyle(
              color: green ? const Color(0xFF19D36B) : Colors.white,
              fontSize: 16,
            ),
          ),
        ],
  ...
  Widget vehiclePage() {
    return ListView(
      ...
      children: <Widget>[
          ...
          children: <Widget>[
            ...
            Expanded(
              child: _actionButton(
                ...
                connected ? disconnect : (found ? connect : () {}),
              ),
            ),
          ],
        ),
        ...
        if (lastCommand.isNotEmpty) ...<Widget>[
          const SizedBox(height: 12),
          _panel(Text(lastCommand, style: const TextStyle(color: Colors.white70))),
        ],
      ],
    );
  }
        ...
        _panel(
          Column(
            ...
            children: <Widget>[
              ...
              const SizedBox(height: 12),
              _line('设备名称', deviceName),
              _line('设备ID', 'TIANKEY-AXELA-01'),
              _line('连接状态', connected ? '已连接' : '未连接'),
              _line('扫描结果', found ? '发现车辆' : '未发现'),
              const SizedBox(height: 10),
  ...
  Widget adminPage() {
    return ListView(
      ...
      children: <Widget>[
        ...
        _panel(
          const Text(
            '授权关闭不会清除管理员会话；管理员可以再次进入本页恢复授权。管理员席位迁移必须通过新的管理员认证完成。',
          ...
          Column(
            children: <Widget>[
              SwitchListTile(
                ...
                subtitle: const Text('已授权管理员设备可自动连接'),
                value: autoConnect,
              ...
              ListTile(
                ...
                trailing: IconButton(
                  onPressed: adminEnabled ? changeDeviceName : null,
                  icon: const Icon(Icons.edit),
                ),
              ),
              ListTile(
                contentPadding: EdgeInsets.zero,
                title: const Text('管理员密码'),
                subtitle: const Text('已保存，不显示明文'),
                trailing: IconButton(
                  onPressed: adminEnabled ? changePassword : null,
                  icon: const Icon(Icons.edit),
                ),
              ),
              SwitchListTile(
                ...
                subtitle: Text(authorized ? '有效' : '已关闭'),
                value: authorized,
                onChanged: adminEnabled ? (_) => toggleAuthorization() : null,
              ),
            ],
          ),
        ),
        ...
        _panel(
          Column(
            ...
            children: <Widget>[
              ...
              _line('时间', espTime?.toString() ?? '未同步'),
              _line('车辆', locked ? '已锁定' : '已解锁'),
              _line('日志', '${logs.length}/200'),
            ],
          ),
        ),
        const SizedBox(height: 12),
        _actionButton('测试时间同步失败', Icons.warning_amber, const Color(0xFF5A3B15), () {
          timeFail = !timeFail;
          _message(timeFail ? '已开启时间同步失败测试' : '已关闭时间同步失败测试');
          setState(() {});
        }),
      ],
    );
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          Icon(
            ...
            color: connected ? const Color(0xFF19D36B) : Colors.grey,
            size: 28,
          ),
